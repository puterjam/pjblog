		...
		Ajax({
			...
			oncomplete : function(obj){
				...
				if (i == (Len - 1)) {
					...
					$("button").innerHTML = "下一步 - 清理临时文件, 完成安装";
					$("button").href = "step_5.asp";
					canSart = false;
...
****************************************/
  ...
  function FollowBox(obj, obj2){
	  ...
	  $(obj2).style.cssText += ";left:" + Ps.x + "px; top:" + (objHeight - 338) + "px;";
  }
  ...
  function Idisplay(c){
	  ...
	  if (t == "open"){
		  ...
		  for (i = 0 ; i < h.length ; i++){
			  if (($(h[i]).style.display == "") || ($(h[i]).style.display == "block")){
  ...
  function doCheck(obj){
	  var len = obj.installType.length, s = "";
	  for (var i = 0 ; i < len ; i++){
		  if (obj.installType[i].checked){s = obj.installType[i].value}
	  }
	  if (s.length > 0){
		  if (s.toLowerCase() == "new"){
			  if (obj.dbfolder.value.length <= 0){
				  alert("数据库文件夹名不能为空")
				  return false;
			  }
			  if (obj.dbname.value.length <= 0){
				  alert("数据库名称不能为空")
				  return false;
			  }
			  if (obj.adminuser.value.length <= 0){
				  alert("管理员名称不能为空")
				  return false;
			  }
			  if (obj.cookieName.value.length <= 0){
				  alert("设置cookiename不能为空")
				  return false;
			  }
			  if (obj.adminpwd.value.length > 0){
				  if (obj.adminpwd.value != obj.cookieencode.value){
					  alert("重复密码,两次输入的密码不相同,请重新输入");
					  obj.cookieencode.value = "";
					  obj.adminpwd.select();
					  return false;
				  }
			  }else{alert("管理员密码不能为空");return false;}
			  cookie.SET("insName", "new", 1);
		  }else if (s.toLowerCase() == "update"){
			  cookie.SET("insName", "update", 1);
			  return true;
		  }else{
			  cookie.SET("insName", "none", 1);
			  return false;
		  }
	  }else{
		  cookie.SET("insName", "none", 1);
