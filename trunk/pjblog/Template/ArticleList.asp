<%ST(A)%><$log_Author$><%ST(A)%><$log_viewCount$><%ST(A)%>
   ...
   <%ST(A)%>
		<div class="Content">
			<div class="Content-top"><div class="ContentLeft"></div><div class="ContentRight"></div>
				 <$ShowButton$>
				 <h1 class="ContentTitle"><img src="<$Cate_icon$>" style="margin:0px 2px -4px 0px;" alt="" class="CateIcon"/><a class="titleA" href="<$pLink$>">[隐藏日志]</a><$log_hiddenIcon$></h1>
				 <h2 class="ContentAuthor">作者:<$log_Author$> 日期:<$log_PostTime$></h2>
			</div>
			<div id="log_<$LogID$>"<$ShowStyle$>>
				<div class="Content-body">
   ...
   <%ST(A)%>
		<tr>
			<td valign="top">
			    <a href="default.asp?cateID=<$log_CateID$>" ><img border="0" alt="查看 <$Cate_Title$> 的日志" src="<$Cate_icon$>" style="margin:0px 2px -3px 0px"/></a>
				<a href="<$pLink$>" title="作者:<$log_Author$> 日期:<$log_PostTime$>">[隐藏日志]</a><$log_hiddenIcon$>
			</td>
