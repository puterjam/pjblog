...
Function getart(ByVal strtagsql)
	...
	If act<>"more" And CurPage>1 then
		tgthml=tgthml&"<br/><strong>模式:</strong> <a style='cursor:pointer'  onclick=check('Getarticle.asp?id="&id&"&act=more','related_tag','related_tag') >全部显示[共"&totalPut&"个相关文章]</a></div>"
	ElseIf  act="more" Then
		tgthml=tgthml&"<div style='margin-top:10px;width:350px;'><strong>模式:</strong> <a style='cursor:pointer'  onclick=check('Getarticle.asp?id="&id&"','related_tag','related_tag') >分页显示[共"&totalPut&"个相关文章]</a></div>"
	end if 
...
Function subpage(ByVal totalPut,ByVal mpage,ByVal CurPage,ByVal Perpage)
	...
	If CurPage-5>1 Then 
		tphtml = tphtml & "<a style=""cursor:pointer""  title=""转到第1页"" onclick=check(""Getarticle.asp?id="&id&"&page=1"",""related_tag"",""related_tag"") >[1]</a>..."
	End If 
	For pi=CurPage-5 To CurPage+5
		If pi>0 And pi< mpage+1 Then 
			...
			Else
				tphtml = tphtml & "<a style=""cursor:pointer""  title=""转到第"&pi&"页"" onclick=check(""Getarticle.asp?id="&id&"&page="&pi&""",""related_tag"",""related_tag"") >["&pi&"]</a>"
			End If 
		End If 
	Next
	If CurPage+5<Int(mpage) Then 
		tphtml = tphtml & "...<a style=""cursor:pointer""  title=""转到第"&mpage&"页"" onclick=check(""Getarticle.asp?id="&id&"&page="&mpage&""",""related_tag"",""related_tag"") >["&mpage&"]</a>"
	End If 
	subpage = tphtml
end Function
%>