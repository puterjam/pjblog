...
%>
                <tr id="Catetr_<%=Int(CateRow(0, i))%>" class="ceo">
                    ...
                    <Select name="Cate_icons" onChange="$('CateImg_<%=Int(CateRow(0, i))%>').src=this.options[this.options.selectedIndex].value" style="width:120px;">
                    	<%=CommonTipPic(joinStr, Trim(CateRow(2, i)))(0)%>
                    </Select>
                    </td>
                    <td class="SecTd"><input name="Cate_Name" type="text" class="text" value="<%=Trim(CateRow(3, i))%>" size="14"/></td>
                    <td class="SecTd"><input name="Cate_Intro" type="text" class="text" value="<%=Trim(CateRow(4, i))%>" size="20"/></td>
                    <td class="SecTd"><input name="Cate_Part" type="text" class="text" value="<%=Trim(CateRow(5, i))%>" size="16" onblur="ReplaceInput(this, window.event)" onkeyup="ReplaceInput(this, window.event)" /></td>
                    <td class="SecTd"><input name="cate_URL" type="text" value="<%=Trim(CateRow(6, i))%>" size="30" class="text" <%If Int(CateRow(9, i)) > 0 Then Response.Write "readonly=""readonly"" style=""background:#e5e5e5""" %> /></td>
                    <td class="SecTd">
...
%>
				<tr style=" line-height:25px;" id="AddRowMark">
                	<td colspan="2">&nbsp;</td>
                    <td colspan="8"><div class="AddNew"><a href="javascript:void(0);" class="AddA" onClick="AddNewCateRow()" id="Addbutton">添加新分类</a></div></td>
                </tr>
